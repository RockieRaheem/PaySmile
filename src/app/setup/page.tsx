...
export default function SetupPage() {
  ...
  const handleSaveSettings = () => {
    ...
    setTimeout(() => {
      ...
      localStorage.setItem(
        "donationSettings",
        JSON.stringify({ monthlyLimit, roundUpAmount })
      );
      setIsSaving(false);
      toast({
  ...
  // Example purchase amount that demonstrates round-up
  const purchaseAmount = 9547; // Changed from 9500 to show actual round-up
    ...
    <div className="flex min-h-screen flex-col bg-background text-foreground">
      ...
      <main className="flex-1 space-y-4 p-4">
        <p className="text-center text-sm text-primary">
          Your small change can make a big difference. Set up your donation
          preferences below.
        </p>
        ...
        <Card>
          <CardContent className="flex items-center justify-between p-4">
            <div className="space-y-1">
              <Label htmlFor="auto-roundup" className="text-base font-medium">
                Enable Automatic Round-Ups
              </Label>
              <p className="text-sm text-muted-foreground">
                Automatically round up your daily mobile payments
              </p>
            </div>
              ...
              <div className="flex justify-between">
                {[10, 50, 100].map((amount) => (
                  <Label
                    ...
                    className={`flex h-8 flex-1 cursor-pointer items-center justify-center rounded-full text-sm font-medium transition-colors ${
        ...
        <Card>
          <CardContent className="flex flex-col items-center p-4 text-center">
            ...
            <p className="mb-2 text-2xl font-bold">
              {new Intl.NumberFormat("en-US").format(purchaseAmount)} UGX
            </p>
            <span className="material-symbols-outlined my-1 text-muted-foreground">
              arrow_downward
            </span>
            <p className="font-medium">Rounds up to</p>
            <p className="mb-2 text-2xl font-bold">
              {new Intl.NumberFormat("en-US").format(totalAmount)} UGX
            </p>
            <div className="my-2 h-px w-full bg-border"></div>
            <p className="font-medium">Your donation</p>
            <p className="text-3xl font-bold text-primary">
              {new Intl.NumberFormat("en-US").format(donationAmount)} UGX
            </p>
            <p className="mt-2 text-xs text-muted-foreground">
              This amount goes directly to a verified community project.
            </p>
          </CardContent>
        </Card>
      </main>
      ...
      <div className="sticky bottom-0 space-y-2 bg-background p-4 pt-0">
        <Button
          size="lg"
          className="h-12 w-full rounded-full font-bold shadow-lg"
